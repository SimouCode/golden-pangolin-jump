...
const SmartRecommendations = () => {
  ...
  const recommendations = useMemo(() => {
    ...
    if (transactions.length === 0) {
      ...
      if (netSavings > 500) { // Arbitrary "high" savings threshold
        tips.push(t('recommendation_great_savings'));
      } else if (netSavings > 0) {
        tips.push(t('recommendation_positive_savings'));
      } else if (netSavings < -100) { // Arbitrary "high" overspending threshold
    ...
    budgets.forEach((budget) => {
      if (budget.monthYear === currentMonthYear) {
        ...
        if (spent > budget.amount * 1.1) { // 10% over budget
          tips.push(t('recommendation_over_budget', { category: budget.category }));
    ...
    <Card>
      ...
      <CardContent className="space-y-2">
        {recommendations.map((tip, index) => (
          <Alert key={index} className="bg-muted/50 border-l-4 border-primary">
